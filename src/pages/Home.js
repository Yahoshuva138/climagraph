...
const Home = () => {
  ...
  const { fetchWeatherData, fetchWeatherByCoords, loading } = useWeather();
  ...
  const handleMapPin = async (place) => {
    // If place contains coordinates, fetch by coords to avoid "City not found" errors
    if (place && place.lat != null && place.lng != null) {
      setCity(place.city || `${place.lat.toFixed(4)}, ${place.lng.toFixed(4)}`);
      try {
        await fetchWeatherByCoords(place.lat, place.lng);
        navigate("/weather");
        toast.success(`Weather data loaded for pinned location`);
      } catch (err) {
        console.error(err);
        toast.error("Failed to fetch weather for pinned location");
      }
      return;
    }
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="max-w-2xl mx-auto mb-12">
              ...
              {/* Map Pin Search */}
              <MapPinSearch onSelect={handleMapPin} />
            </div>
          </div>
        </div>
      </section>
